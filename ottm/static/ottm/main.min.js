"use strict";class Config{#mappings;constructor(data){this.#mappings=data;}
...
class OTTM{#config;#languages;constructor(){this.#config=new Config(window.OTTM_CONFIG.config);this.#languages=[];for(const langData of window.OTTM_CONFIG.languages){this.#languages.push(new Language(langData.code,langData.name,langData.writingDirection,langData.comma,langData.and,langData.dayNames,langData.abbrDayNames,langData.monthNames,langData.abbrMonthNames,langData.ampm,langData.decimalSep,langData.thousandsSep,));}
delete window.OTTM_CONFIG;$("#ottm-config-script").remove();this.#hookSettingsDropdownBehavior();this.#hookDarkModeCallback();this.#hookLanguageSelectorCallback();this.setReferer();this.setAccessKeys();}
...
static getCookie(name,defaultValue){return Cookies.get(name)??defaultValue;}
static setCookie(name,value,options){Cookies.set(name,value,options);}
static deleteCookie(name,options){Cookies.remove(name,options);}}
