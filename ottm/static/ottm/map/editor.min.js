import*as L from"../libs/leaflet/leaflet-src.esm.js";import{Editable}from"../libs/leaflet/plugins/Leaflet.Editable.js";import"../libs/leaflet/plugins/leaflet.snap.js";import"../libs/split.min.js";class SnapData{feature;to;draggedVertex=null;latlng;constructor(feature,to,draggedVertex,latlng){this.feature=feature;this.to=to;this.draggedVertex=draggedVertex;this.latlng=latlng;}}
export class MapEditor{#map;#selectedFeature=null;#layerClicked=false;#isFeatureMenuVisible=false;#drawing=null;#snapData=null;#editorPanel=new MapEditorPanel("editor-panel");Marker;VertexMarker;Polyline;Polygon;constructor(){const thisEditor=this;const setSelected=function(selected){const v=this.hasOwnProperty("_icon")?this._icon:this._path;if(selected){L.DomUtil.addClass(v,"layer-selected");}else{L.DomUtil.removeClass(v,"layer-selected");}};const getVertexMarker=function(latlng){for(const marker of this._markers){if(marker.getLatLng().equals(latlng)){return marker;}}
return null;}
this.Marker=L.Marker.extend({initialize:function(latlngs,options){L.Marker.prototype.initialize.call(this,latlngs,options);this.on("click",()=>{thisEditor.#updateSelection(this);});this.on("contextmenu",()=>{thisEditor.#onContextMenu(this);});},setSelected:setSelected,});this.VertexMarker=Editable.VertexMarker.extend({initialize:function(latlng,latlngs,editor,options){Editable.VertexMarker.prototype.initialize.call(this,latlng,latlngs,editor,options);this._pinnedTo=new Set();this.on("click",()=>{thisEditor.#updateSelection(this);});this.on("contextmenu",()=>{thisEditor.#onContextMenu(this);});},setSelected:function(selected){if(!this._icon){return;}
setSelected.call(this,selected)},onDrag:function(e,propagate=true){console.log(this,propagate);Editable.VertexMarker.prototype.onDrag.call(this,e);if(propagate){this._pinnedTo.forEach(v=>{v.onDrag(e,false);});}},pinTo:function(vertex){if(this._pinnedTo.has(vertex)){return;}
...
this.#onContextMenu(null);});map.on("editable:created",e=>addSnapGuide(e.layer));map.on("editable:vertex:dragstart",e=>{const feature=e.layer;const vertex=e.vertex;removeSnapGuide(feature);snapHandler.watchMarker(vertex);vertex.on("snap",ev=>{this.#onSnap(feature,vertex,ev.latlng,ev.layer);});vertex.on("unsnap",_=>this.#onUnsnap());});map.on("editable:vertex:dragend",e=>{addSnapGuide(e.layer);snapHandler.unwatchMarker(e.vertex);e.vertex.off("snap");e.vertex.off("unsnap");L.DomUtil.removeClass(e.vertex._icon,"marker-snapped");if(this.#snapData?.draggedVertex){this.#snapData.draggedVertex._latlng.lat=this.#snapData.latlng.lat;this.#snapData.draggedVertex._latlng.lng=this.#snapData.latlng.lng;}
