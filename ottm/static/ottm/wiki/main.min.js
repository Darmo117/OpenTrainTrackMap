...
class WikiGadgetManager{#gadgetsQueueSize;#gadgets;#finishedRegistration;#locked;constructor(){this.#gadgetsQueueSize=0;this.#gadgets={};this.#finishedRegistration=false;this.#locked=false;}
registerGadget(gadgetName){if(this.#locked){throw new Error("Cannot register gadgets when manager is locked");}
console.log(`Registering gadget "${gadgetName}…"`);this.#gadgetsQueueSize++;const apiPath=ottm.config.get("wApiPath");$.get(apiPath,{action:"query",query:"gadget",title:encodeURIComponent(gadgetName),},data=>{try{const gadget=eval(data);this.#gadgets[gadget.name]=gadget;}catch(e){console.error(`Error while loading gadget "${gadgetName}":\n${e}`);}
...
followPage(title){}
unfollowPage(title){}}
(function(){window.wiki={gadgetsManager:new WikiGadgetManager(),api:new WikiAPI(),};$(".hljs").each((_,element)=>hljs.highlightElement(element));})();