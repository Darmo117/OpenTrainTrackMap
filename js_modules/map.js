...
// https://github.com/korywka/mapbox-controls/tree/master/packages/zoom
import ZoomControl from "@mapbox-controls/zoom";
import "@mapbox-controls/zoom/src/index.css";
// https://github.com/korywka/mapbox-controls/tree/master/packages/styles
import StylesControl from "@mapbox-controls/styles";
import "@mapbox-controls/styles/src/index.css";
// https://github.com/korywka/mapbox-controls/tree/master/packages/compass
...
(function () {
  const mapStyles = [
    buildStyle(
      ottm.translations.get("map.controls.layers.base.osm"),
      "osm",
      "https://a.tile.openstreetmap.org/{z}/{x}/{y}.png",
      'Map data © <a href="https://www.openstreetmap.org/">OpenStreetMap</a> contributors'
    ),
    buildStyle(
      ottm.translations.get("map.controls.layers.base.satellite_esri"),
      "arcgis",
      "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",
      "Tiles © Esri — Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community"
    ),
    buildStyle(
      ottm.translations.get("map.controls.layers.base.satellite_maptiler"),
      "maptiler",
    ...
    buildStyle(
      ottm.translations.get("map.controls.layers.base.satellite_google"),
      "google",
  ...
  map.addControl(new GeocoderControl({
    language: ottm.getPageLanguage().code,
    searchButtonTitle: ottm.translations.get("map.controls.search.search_button.title"),
    eraseButtonTitle: ottm.translations.get("map.controls.search.erase_button.title"),
    placeholderText: ottm.translations.get("map.controls.search.placeholder"),
    noResultsMessage: ottm.translations.get("map.controls.search.no_results"),
    errorMessage: ottm.translations.get("map.controls.search.error"),
  }), "top-left");
  ...
  map.addControl(new StylesControl({ // TODO translate
    styles: styles,
    onChange: style => onStyleChanged(style.styleUrl, true),
    compact: true,
  }), "top-left");
  ...
  map.addControl(new OpenExternalMapControl({
    buttonTitle: ottm.translations.get("map.controls.google_maps_button.tooltip"),
    iconUrl: `${ottm.config.get("staticPath")}ottm/images/icons/GoogleMaps.png`,
  ...
  // noinspection JSCheckFunctionSignatures
  map.addControl(new OpenExternalMapControl({
    buttonTitle: ottm.translations.get("map.controls.bing_maps_button.tooltip"),
    iconUrl: `${ottm.config.get("staticPath")}ottm/images/icons/Bing.ico`,
  ...
  // noinspection JSCheckFunctionSignatures
  map.addControl(new OpenExternalMapControl({
    buttonTitle: ottm.translations.get("map.controls.osm_button.tooltip"),
    iconUrl: `${ottm.config.get("staticPath")}ottm/images/icons/OSM.png`,
  ...
  // noinspection JSCheckFunctionSignatures
  map.addControl(new OpenExternalMapControl({
    buttonTitle: ottm.translations.get("map.controls.ohm_button.tooltip"),
    iconUrl: `${ottm.config.get("staticPath")}ottm/images/icons/OHM.ico`,
  ...
  // noinspection JSCheckFunctionSignatures
  map.addControl(new OpenExternalMapControl({
    buttonTitle: ottm.translations.get("map.controls.ign_compare_button.tooltip"),
    iconUrl: `${ottm.config.get("staticPath")}ottm/images/icons/IGN.ico`,
  ...
  // noinspection JSCheckFunctionSignatures
  map.addControl(new OpenExternalMapControl({
    buttonTitle: ottm.translations.get("map.controls.geohack_button.tooltip"),
    iconUrl: `${ottm.config.get("staticPath")}ottm/images/icons/WikimediaCloudServices.svg`,
   ...
   * @param name {string} Style’s readable name.
   * @param id {string} Style’s internal ID.
   * @param tilesUrlPattern {string} URL pattern to get map tiles.
   * @param attribution {string} String to display in the bottom right of the map.
   * @param maxZoom {number} Max zoom value.
   * @return {Object} The style object.
   */
  function buildStyle(name, id, tilesUrlPattern, attribution, maxZoom = 19) {
    const type = "raster";
    return {
      name: name,
      version: 8,
  ...
  function patchCssClasses() {
    ...
    for (const [fromClass, toClass] of Object.entries(classMappings)) {
      $(`.${fromClass}:not(.${toClass})`).each((_, e) => $(e).addClass(toClass));
    }
  }
   ...
   * @param shouldUpdateUrlHash {boolean} Whether to update the URL hash.
   */
  function onStyleChanged(style, shouldUpdateUrlHash = false) {
    map.setMaxZoom(style.sources.tiles.maxzoom);
