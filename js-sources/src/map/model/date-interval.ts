...
export class PartialDate {
  ...
  static parse(s: string): PartialDate {
    ...
    if (!match?.groups) throw new Error(`Invalid partial date string: ${s}`);
    const year = +match.groups[1];
    const month = match.groups[2];
    const day = match.groups[3];
    if (month !== "??") {
      if (day !== "??") return new this(year, +month, +day);
  ...
  constructor(year: number, month?: number, day?: number) {
    ...
    if (monthDef) {
      ...
      if (dayDef) PartialDate.#checkDate(year, month, day);
    }
    this.#year = year;
   ...
   */
  toString(): string {
    const year = this.year.toLocaleString("en-US", {
      minimumSignificantDigits: 4,
    });
    const month =
      this.month?.toLocaleString("en-US", { minimumSignificantDigits: 2 }) ??
      "??";
    const day =
      this.day?.toLocaleString("en-US", { minimumSignificantDigits: 2 }) ??
      "??";
...
export class DateInterval {
  ...
  static parse(s: string): DateInterval {
    ...
    if (!m?.groups) throw new Error(`Invalid date interval string: ${s}`);
   ...
   */
  static #extractDatetime(
    match: RegExpMatchArray,
    index: number,
  ): [boolean, PartialDate | null] {
    if (!match.groups) throw new Error("No match");
    let s = match.groups[index];
    if (s === "?") return [false, null];
    const approx = s.startsWith("~");
    if (approx) s = s.substring(1);
  ...
  constructor(
    ...
    if (approxStart && !startDate)
      throw new Error(
        "approxStart cannot be true while startDate is undefined",
      );
  ...
  equals(other: DateInterval): boolean {
    return (
      this.hasApproxStartDate === other.hasApproxStartDate &&
  ...
  precedes(other: DateInterval): boolean {
    return (
      !!this.endDate &&
      !!other.startDate &&
      this.endDate.precedes(other.startDate)
    );
  }
  ...
  follows(other: DateInterval): boolean {
    return (
      !!this.startDate &&
      !!other.endDate &&
      this.startDate.follows(other.endDate)
    );
  }
  ...
  overlaps(other: DateInterval): boolean {
    ...
    return (
      (!!this.startDate &&
        !!thisEnd && // Other starts or ends inside this
        ((!!other.startDate &&
          this.startDate.precedesOrEquals(other.startDate) &&
          other.startDate.precedesOrEquals(thisEnd)) ||
          (!!otherEnd &&
            this.startDate.precedesOrEquals(otherEnd) &&
            otherEnd.precedesOrEquals(thisEnd)))) ||
      (!!other.startDate &&
        !!otherEnd && // This starts or ends inside other
        ((!!this.startDate &&
          other.startDate.precedesOrEquals(this.startDate) &&
          this.startDate.precedesOrEquals(otherEnd)) ||
          (!!thisEnd &&
            other.startDate.precedesOrEquals(thisEnd) &&
            thisEnd.precedesOrEquals(otherEnd))))
    );
  }
  ...
  toString(): string {
    ...
    if (this.startDate && this.hasApproxStartDate) start = "~" + start;
    let end: string;
    if (this.isCurrent) end = "...";
    else {
      end = this.endDate?.toString() ?? "?";
      if (this.endDate && this.hasApproxEndDate) end = "~" + end;
    }
    return `[${start}, ${end}]`;
  }
}
