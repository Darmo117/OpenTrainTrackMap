...
export class Language {
  ...
  constructor(
    ...
    if (dayNames.length !== 7) throw new Error("dayNames expected 7 values");
  ...
  formatNumber(n: number): string {
    ...
    const intPart = Math.floor(n)
      .toString()
      .replace(/\B(?=(\d{3})+(?!\d))/g, this.thousandsSep);
  ...
  listToText(values: unknown[]): string {
    ...
    const list = values.slice(0, values.length - 1).join(this.comma);
...
export class OTTM {
   ...
   * @throws Error If the language ID is missing or undefined.
   */
  getPageLanguage(): Language {
    const langId = this.page.get("language");
    if (typeof langId !== "string") throw new Error("Missing language");
    const lang = this.languages.get(langId);
    if (!lang) throw new Error(`Undefined language "${langId}"`);
    return lang;
  }
  ...
  setReferrer(): void {
    ...
    const linkSelectors = ["#nav-login-link", "#nav-logout-link"];
    for (const linkSelector of linkSelectors) {
      const $link = $(linkSelector);
      if ($link.length) this.#setReturnTo($link, path);
    }
  }
  ...
  #setReturnTo($link: JQuery, path: string, args?: Dict): void {
    const url = new URL($link.prop("href") as string);
    const params = new URLSearchParams({ return_to: path });
    if (args)
      for (const [k, v] of Object.entries(args))
        params.append(k, v ? v.toString() : "");
