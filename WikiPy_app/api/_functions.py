...
from .. import models, skins, settings, special_pages, forms
...
def create_user(username: str, is_male: bool = None, email: str = None, password: str = None, ip: str = None,
    ...
    try:
        forms.username_validator(username, anonymous=anonymous)
    ...
    if anonymous:
        email = None
        is_male = None
    elif not ignore_email:
        try:
            forms.email_validator(email)
...
def get_actual_page_title(raw_title: str) -> str:
    ...
    elif not settings.CASE_SENSITIVE_TITLE and namespace_id not in (6, 7):
